<script lang="ts">
  ...
  let oldItems: (SelectItem & { remove: boolean; room_storage: string })[] = []
  let newItems: SelectItem[] = []

  onMount(async () => {
    const res = await fetch(`/api/room/storages?room_id=${room_id}`)
  ...
  const debouncedFetch = debounce(async (value: string) => {
    const res = await fetch(`/api/storage/search?query=${value}`)
...
{:else}
  ...
  >
    {#each oldItems as { room_storage, value, label, remove } (value)}
      {@const name = remove ? 'remove_storages' : undefined}
      <input type="hidden" {name} value={room_storage} />
      <aside
    ...
    {#each newItems as { value, label } (value)}
      <input type="hidden" name="add_storages" {value} />
