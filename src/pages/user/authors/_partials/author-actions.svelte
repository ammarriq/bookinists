<script lang="ts">
  ...
  const editAuthor: FormEventHandler<HTMLFormElement> = async (e) => {
    submitting = true
    ...
    submitting = false
    const json = (await res.json()) as FetchResponse<Omit<Author, 'icon'>>
  ...
  const deleteAuthor = async () => {
    if (!confirm('Are you sure you wanna delete the author?')) return
    submitting = true
    ...
    submitting = false
    const json = (await res.json()) as FetchResponse<Author>
    if (!json.success) return (errors = json.errors)

    dispatch('delete', author.id)
  }
</script>
  ...
  <Dialog.Portal>
    ...
    >
      ...
      <Dialog.Title class="space-y-1 mb-4">
        <h2 class="text-base font-semibold">Edit Author</h2>
      </Dialog.Title>

      <form
        action="/api/author?edit"
        method="post"
        class="space-y-4"
        on:submit|preventDefault={editAuthor}
      >
        <input type="hidden" name="id" value={author.id} />
        <FileDropzone
          name="avatar"
