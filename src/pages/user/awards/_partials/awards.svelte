<script lang="ts">
  import type { Award as IAward } from '@/pages/api/award'

  import EmptyPlaceholder from '@/components/empty-placeholder.svelte'
  import Pagination from '@/components/pagination.svelte'
  import AddAward from './add-award.svelte'
...
<section class="bg-white px-6 py-4 rounded-md">
  ...
  <ul class="mt-4 overflow-x-auto grid sm:grid-cols-2 md:grid-cols-3 gap-2">
    {#each awardList as award (award.id)}
      <Award
        {award}
        on:edit={(e) => {
          const awardIdx = awardList.findIndex((o) => o.id === e.detail.id)
          const award = { ...awardList[awardIdx], ...e.detail }
          awardList[awardIdx] = award
        }}
        on:delete={(e) => {
          awardList = awardList.filter((o) => o.id !== e.detail)
        }}
      />
    {:else}
      <EmptyPlaceholder
...
<AddAward
  bind:dialogOpen
  on:submit={(e) => (awardList = [...awardList, e.detail])}
/>
