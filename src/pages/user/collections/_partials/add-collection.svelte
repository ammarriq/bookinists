<script lang="ts">
  ...
  let errors: Record<keyof Collection, string[]> | null = null

  const dispatch = createEventDispatcher<{ submit: Collection }>()
  ...
  const submit: FormEventHandler<HTMLFormElement> = async (e) => {
    ...
    submitting = false
    const json = (await res.json()) as FetchResponse<Omit<Collection, 'image'>>
  ...
  <Dialog.Portal>
    ...
    >
      ...
      <Dialog.Title class="space-y-1 mb-4">
        <h2 class="text-base font-semibold">Add Collection</h2>
      </Dialog.Title>

      <form
        action="/api/collection?add"
        method="post"
        class="space-y-4"
