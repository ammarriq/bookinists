<script lang="ts">
  ...
  import AddList from './add-list.svelte'
    ...
    <table class="w-full">
      <thead>
        ...
        >
          ...
          <th class="py-2.5 px-4 whitespace-nowrap">Description</th>
          <th class="py-2.5 px-4 rounded-r-lg whitespace-nowrap">Actions</th>
        </tr>
      </thead>
      <tbody>
        {#each listList as list (list.id)}
          <ListRow
            {list}
            on:edit={(e) => {
              const idx = listList.findIndex((o) => o.id === e.detail.id)
              const list = { ...listList[idx], ...e.detail }
              listList[idx] = list
            }}
            on:delete={(e) => {
              listList = listList.filter((o) => o.id !== e.detail)
            }}
          />
        {:else}
          <tr>
...
<AddList
  bind:dialogOpen
  on:submit={(e) => (listList = [...listList, e.detail])}
/>
