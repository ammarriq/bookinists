<script lang="ts">
  ...
  export let marketplace_id: string | null = null
  ...
  onMount(async () => {
    ...
    const res = await fetch(`/api/marketplace/${marketplace_id}`)
  ...
  const debouncedFetch = debounce(async (value: string) => {
    const res = await fetch(`/api/marketplace/search?query=${value}`)
...
{#if initialLoad}
  <div
    ...
    aria-label="Select a marketplace"
  >
...
{:else}
  <Combobox.Root portal=".dialog" selected={defaultSelected}>
    <Combobox.HiddenInput name="marketplace_id" />
    <Combobox.Input
      type="text"
    ...
    >
      ...
      {:else}
        {#each allItems as marketplace (marketplace.value)}
          <Combobox.Item
