<script lang="ts">
  ...
  export let contact_id: string | null = null
  ...
  onMount(async () => {
    ...
    const res = await fetch(`/api/contact/${contact_id}`)
  ...
  const debouncedFetch = debounce(async (value: string) => {
    const res = await fetch(`/api/contact/search?query=${value}`)
...
{:else}
  <Combobox.Root portal=".dialog" selected={defaultSelected}>
    <Combobox.HiddenInput name="contact_id" />
    <Combobox.Input
      type="text"
    ...
    >
      ...
      {:else}
        {#each allItems as contact (contact.value)}
          <Combobox.Item
