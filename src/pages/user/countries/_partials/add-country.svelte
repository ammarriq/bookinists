<script lang="ts">
  ...
  let errors: Record<keyof Country, string[]> | null = null

  const dispatch = createEventDispatcher<{ submit: Country }>()
  ...
  const submit: FormEventHandler<HTMLFormElement> = async (e) => {
    ...
    submitting = false
    const json = (await res.json()) as FetchResponse<Omit<Country, 'icon'>>
    if (!json.success) return (errors = json.errors)

    dispatch('submit', { ...json.data, flag: flagKey })
    dialogOpen = false
  }
</script>
  ...
  <Dialog.Portal>
    ...
    >
      ...
      <Dialog.Title class="space-y-1 mb-4">
        <h2 class="text-base font-semibold">Add Country</h2>
      </Dialog.Title>

      <form
        action="/api/country?add"
        method="post"
        class="space-y-4"
        on:submit|preventDefault={submit}
      >
        <FileDropzone name="flag" error={errors?.flag} bind:key={flagKey} />
