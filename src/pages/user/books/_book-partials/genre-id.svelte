<script lang="ts">
  ...
  export let genre_id: string | null = null
  ...
  onMount(async () => {
    ...
    const res = await fetch(`/api/genre/${genre_id}`)
  ...
  const debouncedFetch = debounce(async (value: string) => {
    const res = await fetch(`/api/genre/search?query=${value}`)
...
{:else}
  <Combobox.Root portal=".dialog" selected={defaultSelected}>
    <Combobox.HiddenInput name="genre_id" />
    <Combobox.Input
      type="text"
    ...
    <Combobox.Content
      ...
      sideOffset={8}
    >
      {#each allItems as genre (genre.value)}
        <Combobox.Item
      ...
      {:else}
        <p class="text-xs text-center py-1">
          <span class="italic capitalize text-slate-500">no genre</span>
        </p>
      {/each}
    </Combobox.Content>
  </Combobox.Root>
