<script lang="ts">
  ...
  export let award_id: string | null = null
  ...
  onMount(async () => {
    ...
    const res = await fetch(`/api/award/${award_id}`)
  ...
  const debouncedFetch = debounce(async (value: string) => {
    const res = await fetch(`/api/award/search?query=${value}`)
...
{:else}
  <Combobox.Root portal=".dialog" selected={defaultSelected}>
    <Combobox.HiddenInput name="award_id" />
    <Combobox.Input
      type="text"
    ...
    >
      ...
      {:else}
        {#each allItems as award (award.value)}
          <Combobox.Item
