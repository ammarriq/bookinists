<script lang="ts">
  ...
  export let award_category_id: string | null = null
  ...
  onMount(async () => {
    ...
    const res = await fetch(`/api/award-category/${award_category_id}`)
  ...
  const debouncedFetch = debounce(async (value: string) => {
    const res = await fetch(`/api/award-category/search?query=${value}`)
...
{:else}
  <Combobox.Root portal=".dialog" selected={defaultSelected}>
    <Combobox.HiddenInput name="award_category_id" />
    <Combobox.Input
      type="text"
    ...
    >
      ...
      {:else}
        {#each allItems as category (category.value)}
          <Combobox.Item
