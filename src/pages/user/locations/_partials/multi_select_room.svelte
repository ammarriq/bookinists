<script lang="ts">
  ...
  let oldItems: (SelectItem & { remove: boolean; location_room: string })[] = []
  let newItems: SelectItem[] = []

  onMount(async () => {
    const res = await fetch(`/api/location/rooms?location_id=${location_id}`)
  ...
  const debouncedFetch = debounce(async (value: string) => {
    const res = await fetch(`/api/room/search?query=${value}`)
...
{:else}
  ...
  >
    {#each oldItems as { location_room, value, label, remove } (value)}
      {@const name = remove ? 'remove_rooms' : undefined}
      <input type="hidden" {name} value={location_room} />
      <aside
    ...
    {#each newItems as { value, label } (value)}
      <input type="hidden" name="add_rooms" {value} />
