<script lang="ts">
  import type { AwardCategory as ICategory } from '@/pages/api/award-category'

  import EmptyPlaceholder from '@/components/empty-placeholder.svelte'
  import Pagination from '@/components/pagination.svelte'
  import AddCategory from './add-category.svelte'
...
<section class="bg-white px-6 py-4 rounded-md">
  <header class="grid md:grid-cols-[auto_1fr] items-center">
    <h2 class="font-semibold">Award categories</h2>
  ...
  <ul class="mt-4 overflow-x-auto grid sm:grid-cols-2 md:grid-cols-3 gap-2">
    {#each categoryList as category (category.id)}
      <Category
        {category}
        on:edit={(e) => {
          const idx = categoryList.findIndex((o) => o.id === e.detail.id)
          const categories = { ...categoryList[idx], ...e.detail }
          categoryList[idx] = categories
        }}
        on:delete={(e) => {
          categoryList = categoryList.filter((o) => o.id !== e.detail)
        }}
      />
    {:else}
      <EmptyPlaceholder
...
<AddCategory
  bind:dialogOpen
  on:submit={(e) => (categoryList = [...categoryList, e.detail])}
/>
